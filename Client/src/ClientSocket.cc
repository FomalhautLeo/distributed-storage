...
ClientSocket::ClientSocket(int socket) : socket_fd_(socket) {
...
bool ClientSocket::ConnectToServer(
    const std::string& ip, unsigned short port, bool silence) {
  ...
  if (ret == -1) {
    // Allot a socket fd again.
    close(socket_fd_);
    socket_fd_ = socket(AF_INET, SOCK_STREAM, 0);
    ret = connect(socket_fd_, (sockaddr*)&sock_addr, sizeof(sock_addr));
    if (!silence && ret == -1)
      LogError << "Failed to connect to server." << std::endl;
    return false;
  }
  return true;
}
...
int ClientSocket::SendMsg(const msg_t msg_type){
  std::string str_msg(1, msg_type);
  str_msg += "\n";
  return SendMsg(str_msg);
}

int ClientSocket::SendMsg(const msg_t msg_type, const std::string& str_param) {
  std::string str_msg(1, msg_type);
  str_msg += "\t" + str_param +"\n";
  return SendMsg(str_msg);
}

int ClientSocket::SendMsg(
...
std::string ClientSocket::RecvMsg(bool flag) {
